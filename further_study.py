...
def custom_reverse(input_list):
    ...
    for index in range(list_len):
        pop_elem = custom_pop(input_list)
        custom_append(new_list, pop_elem)
...
def custom_equality(some_list, another_list):
    ...
    """
    some_list_len = custom_len(some_list)
    another_list_len = custom_len(another_list)

    if some_list_len != another_list_len:
        return False

    for index in range(some_list_len -1):
